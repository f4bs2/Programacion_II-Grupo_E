...
#     with open("paciente.txt", "w",encoding="utf-8" )as archivo: #la "w" es un modo de apertura del archivo para guardar(write)
#         for paciente in paciente_data:
#             archivo.write(
#                 f"{paciente['Nombre Completo']}|"
...
#             cargar_en_archivo()   #guardar cambios en archivo
#             cargar_treeview()
...
# text="Eliminar", command=eliminar_paciente, bg="red", fg="white")
...
def eliminar_doctor():
    ...
    if seleccionado:
        indice=int(seleccionado[0])
        id_item=seleccionado[0]
        if messagebox.askyesno("Eliminar Doctor", f"¿Esta seguro de eliminar el paciente'{treeview.item(id_item,'values')[0]}'?"):
            del doctor_data[indice]
            cargar_en_archivo2()   #guardar cambios en archivo
            cargar_treeview2()
            messagebox.showinfo("Eliminar Paciente","Paciente Eliminado Exitosamente.")
        else:
            messagebox.showwarning("Eliminar Paciente","No se ha seleccionado ningun Paciente.")
...
# Botón Eliminar
btn_eliminar = tk.Button(btn_frame,
text="Eliminar", command="Eliminar", command=eliminar_doctor, bg="red", fg="white")
# btn_eliminar.grid(row=0, column=1, padx=5), bg="red", fg="white")
